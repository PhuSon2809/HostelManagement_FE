...
export const getHostelByAccountId = (accountId, filters) => {
  return (dispatch) => {
    ...
    if (token) {
      ...
      axiosClient
        .getWithIdFilterMiddleParams(
          `/Hostels?accountId=${accountId}&pageIndex=${filters.pageIndex}&pageSize=${filters.pageSize}`
        )
        .then((response) => {
          if (response.data) {
...
export const getHostelById = (hostelId) => {
  ...
  return (dispatch) => {
    ...
    if (token) {
      axiosClient.setHeaderAuth(token);
      axiosClient
        .getWithIdFixBug(`/Hostels`, hostelId)
        .then((response) => {
          if (response) {
            console.log("response: ", response);
